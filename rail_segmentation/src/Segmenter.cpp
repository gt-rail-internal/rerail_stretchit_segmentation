...
bool Segmenter::segmentObjects(rail_manipulation_msgs::SegmentedObjectList &objects)
{
  ...
  ROS_INFO("Inside Segmenter::segmentObjects()");
  ...
  while (point_cloud_time < request_time)
  {
    ...
    {
      ROS_INFO("Got point cloud for segmentation.");
      *pc = *pc_msg;
      break;
    }
    point_cloud_time = pcl_conversions::fromPCL(pc->header.stamp);
    ROS_INFO("point_cloud_time: %f", point_cloud_time.toSec());
  }
  ROS_INFO("returning from Segmenter::segmentObjects()");
